...
function reminderCategoryLabel(value) {
  return REMINDER_CATEGORIES.find(([key]) => key === value)?.[1] || "📌 Geral";
}
...
export default function LembretesPage() {
  ...
  const [editingCategoria, setEditingCategoria] = useState("pessoal");
  ...
  function getReminderDate(item) {
    if (item?.tipo === "avulso") return parseLocalDateTime(item.quando);
    const d = new Date(item?.nextDueISO || "");
    return Number.isNaN(d.getTime()) ? null : d;
  }

  function postponeReminder(id, mode) {
    const now = new Date();
    const next = list.map((item) => {
      ...
      let newDate;
      if (mode === "1h") {
        const current = getReminderDate(item);
        newDate = new Date(Math.max(Date.now(), current?.getTime?.() || 0) + 60 * 60 * 1000);
      ...
      if (item.tipo === "avulso") {
        return {
          ...item,
          quando: toDatetimeLocal(newDate),
          done: false,
          doneAt: null,
          updatedAt: nowISO(),
        };
      ...
      return {
        ...item,
        nextDueISO: newDate.toISOString(),
        enabled: true,
        updatedAt: nowISO(),
      };
    });
    ...
    toastMsg(
      mode === "1h"
        ? "⏰ Adiado por 1 hora."
        : mode === "tomorrow"
        ? "⏰ Adiado para amanhã às 09:00."
        : "⏰ Adiado por 1 semana."
    );
  }

  function setQuickWhen(mode) {
    const now = new Date();
    let d;
    if (mode === "todayEvening") {
      d = new Date(now);
      d.setHours(19, 0, 0, 0);
      if (d.getTime() <= Date.now()) d = addDays(d, 1);
    } else if (mode === "tomorrow") {
      d = addDays(startOfDay(now), 1);
      d.setHours(9, 0, 0, 0);
    } else if (mode === "nextWeek") {
      d = addDays(startOfDay(now), 7);
      d.setHours(9, 0, 0, 0);
    }
    if (d) setQuando(toDatetimeLocal(d));
  }
  ...
  function startEdit(item, e) {
    ...
    setEditingCategoria(item.categoria || "pessoal");
    ...
    const next = list.map((i) => {
      ...
      if (newTipo === "avulso") {
        ...
        return {
          ...
          conflictMode: cm,
          categoria: editingCategoria || "pessoal",
          updatedAt: nowISO(),
        };
      }
  ...
  const visible = useMemo(() => {
    ...
    if (range !== "all") {
      ...
      base = base.filter((i) => {
        if (i.tipo === "avulso" && i.done) return range === "completed";
        if (i.tipo === "recorrente" && i.enabled === false && range !== "completed") return false;

        const dt =
          i.tipo === "avulso"
            ? parseLocalDateTime(i.quando)
            : new Date(i.nextDueISO || "");
        if (!dt || Number.isNaN(dt.getTime())) return false;

        if (range === "today") return dt >= todayStart && dt <= todayEnd;
        if (range === "tomorrow") return dt >= tomorrowStart && dt <= tomorrowEnd;
        if (range === "week") return dt >= todayStart && dt <= weekEnd;
        if (range === "month") return dt >= todayStart && dt <= monthEnd;
        if (range === "late") return dt < todayStart && !(i.tipo === "avulso" && i.done);
        if (range === "upcoming") return dt > tomorrowEnd;
        if (range === "completed") return i.tipo === "avulso" ? !!i.done : i.enabled === false;
        return true;
      });
    }
    ...
    if (q) {
      base = base.filter(
        (i) =>
          normalizeText(i.titulo).includes(q) ||
          normalizeText(reminderCategoryLabel(i.categoria)).includes(q)
      );
    }
  ...
  const reminderCounts = useMemo(() => {
    ...
    list.forEach((i) => {
      if (i.tipo === "avulso" && i.done) return;
      if (i.tipo === "recorrente" && i.enabled === false) return;
      const dt = getReminderDate(i);
      if (!dt) return;
      if (dt >= todayStart && dt <= todayEnd) today++;
      if (dt >= todayStart && dt <= weekEnd) week++;
      if (dt >= todayStart && dt <= monthEnd) month++;
    });
  ...
  return (
    <div className="page">
      ...
      <Modal open={addModalOpen} title="Novo lembrete" onClose={() => setAddModalOpen(false)}>
        {maisOpcoes ? (
          <>
        <strong style={{ display: "block", marginBottom: 8 }}>⚙️ Ajustes extras</strong>
        <div className="filters-grid">
          <div className="field">
            <label>Como lembrar</label>
        ...
        <div className="filters-grid mt">
          ...
          <div className="field">
            <label>Se já houver outro no mesmo dia</label>
        ...
        <div className="field mt">
          ...
          <input value={titulo} onChange={(e) => setTitulo(e.target.value)} placeholder='Ex.: Fazer atividade de Ciências' />
          <div className="muted small" style={{ marginTop: 6 }}>
            {voiceSupported ? (
              <>
        ...
        <div className="field" style={{ marginTop: 12 }}>
          <label>Em qual parte da vida?</label>
          <select value={categoria} onChange={(e) => setCategoria(e.target.value)}>
            {REMINDER_CATEGORIES.map(([value, label]) => (
              <option key={value} value={value}>{label}</option>
            ))}
          </select>
        </div>

        <div className="chips-row" style={{ marginTop: 10, flexWrap: "wrap" }}>
          <button type="button" className={"chip " + (tipo === "avulso" ? "chip-active" : "")} onClick={() => setTipo("avulso")}>
            Uma vez
          </button>
          <button type="button" className={"chip " + (tipo === "recorrente" ? "chip-active" : "")} onClick={() => setTipo("recorrente")}>
            Repetir
          </button>
          {tipo === "recorrente" ? (
            <>
              <button type="button" className="chip" onClick={() => setScheduleType("diario")}>Todo dia</button>
              <button type="button" className="chip" onClick={() => { setScheduleType("semanal"); setWeekdays([1,2,3,4,5]); }}>Dias úteis</button>
              <button type="button" className="chip" onClick={() => setScheduleType("mensal")}>Todo mês</button>
            </>
          ) : null}
        </div>
        ...
        >
          {maisOpcoes ? "▲ Menos opções" : "⚙️ Mais opções"}
        </button>

        <strong style={{ display: "block", marginTop: 16 }}>📅 Quando fazer?</strong>
        {tipo === "avulso" ? (
          <div className="field" style={{ marginTop: 12 }}>
            <label>Data e horário</label>
            <input value={quando} onChange={(e) => setQuando(e.target.value)} type="datetime-local" />
            <div className="chips-row" style={{ marginTop: 8, flexWrap: "wrap" }}>
              <button type="button" className="chip" onClick={() => setQuickWhen("todayEvening")}>Hoje à noite</button>
              <button type="button" className="chip" onClick={() => setQuickWhen("tomorrow")}>Amanhã 09:00</button>
              <button type="button" className="chip" onClick={() => setQuickWhen("nextWeek")}>Próxima semana</button>
            </div>
          </div>
        ) : (
          <>
      ...
      <section className="card mt">
        <strong style={{ display: "block", marginBottom: 10 }}>📌 Afazeres</strong>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, minmax(0,1fr))", gap: 8 }}>
          <button type="button" className={"chip " + (range === "today" ? "chip-active" : "")} onClick={() => { setTab("pending"); setRange("today"); }}>
            <strong>{reminderCounts.today}</strong><br /><span className="muted small">Do dia</span>
          </button>
          <button type="button" className={"chip " + (range === "week" ? "chip-active" : "")} onClick={() => { setTab("pending"); setRange("week"); }}>
            <strong>{reminderCounts.week}</strong><br /><span className="muted small">Da semana</span>
          </button>
          <button type="button" className={"chip " + (range === "month" ? "chip-active" : "")} onClick={() => { setTab("pending"); setRange("month"); }}>
            <strong>{reminderCounts.month}</strong><br /><span className="muted small">Do mês</span>
          </button>
        </div>
        <small className="muted" style={{ display: "block", marginTop: 10 }}>Toque em um botão para ver somente os afazeres daquele período.</small>
      </section>
      ...
      <div className="card mt">
        ...
        <div className="chips-row" style={{ flexWrap: "wrap" }}>
          ...
          <button type="button" className={"chip " + (range === "tomorrow" ? "chip-active" : "")} onClick={() => setRange("tomorrow")}>
            Amanhã
          </button>
          <button type="button" className={"chip " + (range === "late" ? "chip-active" : "")} onClick={() => setRange("late")}>
            Atrasados
          </button>
          <button type="button" className={"chip " + (range === "week" ? "chip-active" : "")} onClick={() => setRange("week")}>
            Esta semana
          </button>
          <button type="button" className={"chip " + (range === "month" ? "chip-active" : "")} onClick={() => setRange("month")}>
            Este mês
          </button>
          <button type="button" className={"chip " + (range === "upcoming" ? "chip-active" : "")} onClick={() => setRange("upcoming")}>
            Próximos
          </button>
                    ...
                    {!isEditing ? (
                      <>
                        ...
                        <div className="muted small" style={{ marginTop: 4 }}>
                          {reminderCategoryLabel(i.categoria)}
                        </div>
                      ...
                      <div>
                        ...
                        <div className="field" style={{ marginBottom: 10 }}>
                          ...
                          <select value={editingCategoria} onChange={(e) => setEditingCategoria(e.target.value)}>
                            {REMINDER_CATEGORIES.map(([value, label]) => (
                              <option key={value} value={value}>{label}</option>
                            ))}
                          </select>
                    ...
                    {!isEditing ? (
                      <>
                        ...
                        <details style={{ position: "relative" }}>
                          <summary className="chip" style={{ width: "auto", cursor: "pointer", listStyle: "none" }}>
                            ⏰ Adiar
                          </summary>
                          <div style={{ display: "flex", flexDirection: "column", gap: 6, marginTop: 6 }}>
                            <button type="button" className="chip" onClick={() => postponeReminder(i.id, "1h")}>+ 1 hora</button>
                            <button type="button" className="chip" onClick={() => postponeReminder(i.id, "tomorrow")}>Amanhã 09:00</button>
                            <button type="button" className="chip" onClick={() => postponeReminder(i.id, "week")}>+ 1 semana</button>
                          </div>
                        </details>
